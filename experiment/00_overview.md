...
**28 eksperimen** = 7 konfigurasi fitur × 4 classifier:
...
DemogPairs 10.800 (6×1.800) → Normalisasi citra 224×224 → 3 ViT → fitur 768 dimensi → arsip fitur
  ...
  → Optimasi 5-Fold (1.086 kombinasi, 5.430 fitting per fitur) → evaluasi 2.160 uji → metrik global + per kelas + matriks konfusi
```
